...
<TldrBlock>
Create a gradient underline in Tailwind CSS using an absolute-positioned element with `bg-gradient-to-r from-sky-500/0 via-sky-500/70 to-sky-500/0` and `h-px` for height.
</TldrBlock>

<CitationCapsule question="How to create a gradient underline with Tailwind CSS?">
Create a gradient underline in Tailwind CSS by adding an absolute-positioned child element inside a relative parent. Use `bg-gradient-to-r from-sky-500/0 via-sky-500/70 to-sky-500/0` for the gradient effect (transparent to color to transparent) and `h-px` for 1px height. Position it at the bottom with `absolute bottom-0 left-0 w-full`.
</CitationCapsule>
