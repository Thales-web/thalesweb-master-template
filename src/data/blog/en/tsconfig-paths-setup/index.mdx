...
<TldrBlock>
TSConfig path aliases replace messy relative imports like `../../../components/` with clean aliases like `@/components/`. Set them up in tsconfig.json with the `paths` option for cleaner, more maintainable TypeScript code.
</TldrBlock>

<CitationCapsule question="What are TSConfig path aliases?">
TSConfig path aliases let you replace long relative import paths (e.g., `../../../components/MyComponent`) with short, readable aliases (e.g., `@/components/MyComponent`). Configure them in your tsconfig.json using the `paths` compiler option. This improves code readability, maintainability, and allows moving files without updating import paths everywhere.
</CitationCapsule>
